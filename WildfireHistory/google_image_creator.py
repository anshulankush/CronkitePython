...
for i in range(get_count()):
    ...
    lat = "{0:.2f}".format(float(xstr(dataset_root[1][i][7]
               .text.replace(" ", ".", 1).replace(" ", ""))))
    lng = "{0:.2f}".format(float(xstr(dataset_root[1][i][8]
               .text.replace(" ", ".", 1).replace(" ", ""))))
    image_url = "http://api.tiles.mapbox.com/v3/examples.map-zr0njcqy/" + lng + "," + lat + ",13/140x140.png"
    print(image_url)
